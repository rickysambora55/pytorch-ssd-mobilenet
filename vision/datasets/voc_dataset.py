    ...
    def __init__(self, root, transform=None, target_transform=None, is_test=False, keep_difficult=False, label_file=None):
        ...
        if is_test:
            image_sets_file = os.path.join(
                self.root, 'ImageSets/Main/test.txt')
        else:
            image_sets_file = os.path.join(
                self.root, 'ImageSets/Main/trainval.txt')

        if not os.path.isfile(image_sets_file):
            image_sets_default = os.path.join(
                self.root, 'ImageSets/Main/default.txt')   # CVAT only saves default.txt
    ...
    def _read_image_ids(self, image_sets_file):
        ...
        with open(image_sets_file) as f:
            for line in f:
                ...
                if len(image_id) <= 0:
                    print(
                        'warning - found empty line in {:s}, skipping line'.format(str(image_sets_file)))
                    continue
                ...
                if self._get_num_annotations(image_id) > 0:
                    if self._find_image(image_id) is not None:
                        ...
                        print(
                            'warning - could not find image {:s} - ignoring from dataset'.format(image_id))
                else:
                    print(
                        'warning - image {:s} has no box/labels annotations, ignoring from dataset'.format(image_id))

        return ids

    def _get_num_annotations(self, image_id):
        annotation_file = os.path.join(
            self.root, f'Annotations/{image_id}.xml')
