...
class MobileBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernal_size, stride, nonLinear, SE, exp_size):
        ...
        self.depth_conv = nn.Sequential(
            nn.Conv2d(exp_size, exp_size, kernel_size=kernal_size,
    ...
    def __init__(self, model_mode="SMALL", num_classes=30, multiplier=1.0, dropout_rate=0.0):
        ...
        if model_mode == "LARGE":
            ...
            self.out_conv1 = nn.Sequential(
                nn.Conv2d(out_conv1_in, out_conv1_out,
            ...
            self.out_conv2 = nn.Sequential(
                nn.Conv2d(out_conv2_in, out_conv2_out,
                          kernel_size=1, stride=1),
                h_swish(inplace=True),
                nn.Dropout(dropout_rate),
                nn.Conv2d(out_conv2_out, self.num_classes,
                          kernel_size=1, stride=1),
            )
            ...
            self.features.append(nn.Conv2d(
                in_channels=3, out_channels=init_conv_out, kernel_size=3, stride=2, padding=1))
            ...
            for in_channels, out_channels, kernal_size, stride, nonlinear, se, exp_size in layers:
                ...
                self.block.append(MobileBlock(
                    in_channels, out_channels, kernal_size, stride, nonlinear, se, exp_size))
                self.features.append(MobileBlock(
                    in_channels, out_channels, kernal_size, stride, nonlinear, se, exp_size))
            self.block = nn.Sequential(*self.block)
