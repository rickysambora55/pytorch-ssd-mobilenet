...
# Tensorboard                    
tensorboard = SummaryWriter(log_dir=os.path.join(args.checkpoint_folder, "tensorboard"))
...
if __name__ == '__main__':
    ...
    if args.net == 'mb1-ssd':
        ...
        create_net = lambda num: create_mobilenetv2_ssd_lite(num, 1)
        config = mobilenetv1_ssd_config
    ...
    if validation_mean_ap:
        eval_dataset = VOCDataset(dataset_path, is_test=True)
        eval = MeanAPEvaluator(eval_dataset, net, arch=net_arch, eval_dir=os.path.join(checkpoint_folder, 'eval_results'))
    ...
    if resume:
        logging.info(f"Resuming from the model {resume}")
        net.load(resume)
    elif pretrained_ssd:
    ...
    optimizer = torch.optim.ASGD(params, lr=lr, weight_decay=args.weight_decay)
                                
    logging.info(f"Learning rate: {args.lr}")

    # Set learning rate policy
    logging.info("Uses CosineAnnealingLR scheduler.")
    scheduler = CosineAnnealingLR(optimizer, num_epochs, last_epoch=last_epoch)
