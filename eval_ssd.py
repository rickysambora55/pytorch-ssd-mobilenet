...
class MeanAPEvaluator:
    ...
    def __init__(self, dataset, net, arch='mb1-ssd', eval_dir='models/eval_results',
        ...
        self.true_case_stat, self.all_gb_boxes, self.all_difficult_cases = self.group_annotation_by_class(
            self.dataset)
        ...
        if arch == 'mb1-ssd':
            ...
            self.predictor = create_mobilenetv2_ssd_lite_predictor(
                net, nms_method=nms_method, device=device)
        elif arch == 'mb3-ssd-lite':
            self.predictor = create_mobilenetv3_ssd_lite_predictor(
                net, nms_method=nms_method, device=device)
        else:
            raise ValueError(
                f"Invalid network architecture type '{arch}' - it should be one of:  mb1-ssd or mb2-ssd-lite")
    ...
    def compute(self):
        ...
        for i in range(len(self.dataset)):
            logging.debug(
                f"evaluating average precision   image {i} / {len(self.dataset)}")
    ...
    def group_annotation_by_class(self, dataset):
        ...
        for class_index in all_difficult_cases:
            for image_id in all_difficult_cases[class_index]:
                all_gt_boxes[class_index][image_id] = all_gt_boxes[class_index][image_id].clone(
                ).detach()  # torch.tensor(all_gt_boxes[class_index][image_id])
...
if __name__ == '__main__':
    ...
    parser.add_argument("--model-dir", "--model", type=str,
    ...
    parser.add_argument("--label_file", "--labels",
    ...
    parser.add_argument("--eval_dir", default="models/eval_results",
    ...
    if args.net == 'vgg16-ssd':
        ...
        net = create_mobilenetv1_ssd_lite(
            len(dataset.class_names), is_test=True)
    elif args.net == 'sq-ssd-lite':
        net = create_squeezenet_ssd_lite(
            len(dataset.class_names), is_test=True)
    elif args.net == 'mb2-ssd-lite':
        net = create_mobilenetv2_ssd_lite(
            len(dataset.class_names), width_mult=args.mb2_width_mult, is_test=True)
    elif args.net == 'mb3-ssd-lite':
        net = create_mobilenetv3_ssd_lite(
            len(dataset.class_names), is_test=True)
    else:
        logging.fatal(
            f"Invalid network architecture type it should be one of:  vgg16-ssd, mb1-ssd, mb1-ssd-lite, mb2-ssd-lite, sq-ssd-lite")
